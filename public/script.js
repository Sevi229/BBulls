...
divmain.id = "textsmall";

const divo = document.createElement("div");
divo.id = "text-padding";
const text = document.createElement("p");
text.innerHTML = "Unfortunately, the minimum screen width required to play the game is 770px. Your current screen width does not meet this requirement, so you will not be able to play the game at this time. We apologize for any inconvenience this may cause and recommend trying again on a device with a larger screen.";
...
add1.appendChild(divmain);
    ...
    // Append the img element to the body element
			...
			imgElement.addEventListener("click", function()  {
                ...
                try {
                  var accountEString = localStorage.getItem('accountE');
                  accountE = JSON.parse(accountEString);
                } catch (error) {
                  accountE = {};
                }
            ...
            setInterval(()=> {
				...
				if(y1 == false && count == value && go == 0 || speed == 1){
                    count = 0;
                   pos[0] = Math.floor(Math.random() * 9);
                   speed = 0;
                   if(pos[0] == 0){ terrorist.setAttribute("src", "./m/" + to[0].toString() + ".png"); aim.setAttribute("src", "./x/" + to[0].toString() + ".png")}
...
async function updateLeaderboardEntry(data) {
  const response = await fetch(`/leaderboard/${data._id}`, {
    method: 'PUT',
    headers: {
...
// Function to delete leaderboard entry
...
// Function to add new leaderboard entry
async function addLeaderboardEntry(points, wallet) {
  const response = await fetch('/leaderboard', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
...
function populateLeaderboardTable(data) {
  ...
  rankCell.textContent = "Rank";
    ...
    leaderboardTable.appendChild(rankCell);
    leaderboardTable.appendChild(pointsCell);
    leaderboardTable.appendChild(walletCell);
  
    let currentRank = 0;
let previousPoints = 0;

data.forEach((entry, index) => {
  const { points, wallet } = entry;
...
if(wallet === accountE) {
        rankCell.style.backgroundColor = "#fcc602";
        pointsCell.style.backgroundColor = "#fcc602";
        walletCell.style.backgroundColor = "#fcc602";
 }
...
async function handleFormSubmission(event) {
    ...
    await addLeaderboardEntry(points, wallet);
    
    populateLeaderboardTable(await fetchLeaderboardData());
}
